...
If Not ftpcom Return 0;RuntimeError( "FTP Connection Failed. Check your internet connection And retry" )
...
DebugLog "Befehl:	USER "+ftp_login$
Case 331
WriteLine ftpcom,"PASS "+ftp_pass$
DebugLog "Befehl:	PASS "+ftp_pass$
Case 332
WriteLine ftpcom,"ACCT "+ftp_acct$
DebugLog "Befehl:	ACCT "+ftp_acct$
Case 530
...
DebugLog "Befehl:	RNFR"
FTP_waitcode(350)

WriteLine ftpcom,"RNTO "+file2$
DebugLog "Befehl:	RNTO"
FTP_waitcode(250)
...
;---------------------------------
Function FTP_Put(Trans_file$,FTP_Wait_of_150,progress,x,y)
;---------------------------------
If Not ftpcom Then FTP_connect(ftp$,FTP_login$,FTP_pass$,FTP_acct$)
...
If FTP_Wait_of_150=1 Then FTP_waitcode(150)
open_data()

WriteLine ftpcom,"STOR "+trans_file$
DebugLog "Befehl:	STOR "+trans_file$
...
;-----------------------------
Function FTP_Get(Trans_file$,FTP_Wait_of_150,progressind,x,y)
;-----------------------------
If Not ftpcom Then FTP_connect(ftp$,FTP_login$,FTP_pass$,FTP_acct$)
...
DebugLog "Befehl:	RETR "+trans_file$


While Not ReadAvail(FTP_data)=0
ReadAvail(FTP_data)
Wend

FTP_Savefile(Trans_file$,progressind,x,y,trans_FileSize)
...
If Not FTP_msg$="" Then DebugLog "Antwort:	"+FTP_msg$
...
DebugLog "Befehl:	RETR "+files$+" "+Str$(user)+Str$(group)+Str$(other)
ftp_waitcode(200)
...
Function FTP_Quit()
;----------------------------
If Not ftpcom=0 Then WriteLine ftpcom,"QUIT"
DebugLog "Befehl:	QUIT"
FTP_waitcode(221)
